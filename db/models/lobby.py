...
class Crew(me.EmbeddedDocument):
    ...
    crewmates = me.ListField(me.ReferenceField('User'))
...
class Lobby(me.Document):
    ...
    def remove_from_crews(self, the_user: 'User') -> None:
    ...
    def add_to_crew(self, the_user: 'User', crew_id: str) -> None:
        assert the_user.current_lobby == self
    ...
    def add_user(self, the_user: 'User', password: Optional[str]) -> None:
        if the_user.current_lobby is not None:
            raise error.SpbError(
                f'User {the_user.get_display_name()} is already in a lobby',
    ...
    def remove_user(self, the_user: 'User') -> None:
    ...
    def to_dict(self) -> dict:
        ...
        return dict(
            ...
            crews=[crew.to_dict() for crew in self.crews],
